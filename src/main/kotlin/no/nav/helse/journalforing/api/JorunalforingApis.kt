...
fun Route.journalforingApis(
        ...
        when {
            call.request.gjelderFrilanserOgSelvstendigNæringsdrivende() -> {
                ...
                val metadata = MetadataV1(
                    ...
                    søknadstype = Søknadstype.OMSORGSPENGESØKNAD_UTBETALING_FRILANSER_SELVSTENDIG
                )
                journalfør(journalforingV1Service, melding, metadata)
            }
            call.request.gjelderArbeidstaker() -> {
                ...
                val metadata = MetadataV1(
                    ...
                    correlationId = call.request.getCorrelationId(),
                    requestId = call.response.getRequestId(),
                    søknadstype = Søknadstype.OMSORGSPENGESØKNAD_UTBETALING_ARBEIDSTAKER)
                journalfør(journalforingV1Service, melding, metadata)
            }
            else -> {
    ...
    post("/v1/opplæringspenge/journalforing") {
        ...
        val metadata = MetadataV1(
            ...
            correlationId = call.request.getCorrelationId(),
            requestId = call.response.getRequestId(),
            søknadstype = Søknadstype.OPPLÆRINGSPENGESØKNAD
        )
        journalfør(journalforingV1Service, melding, metadata)
    }
}
